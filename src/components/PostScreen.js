    ...
    <VStack my="2" key={post.id} width="100%">
      ...
      <VStack space={1} px="3" width="100%">
        ...
        <Pressable onPress={handleOpenLikers}>
          <Text fontWeight="light">Voir les XXX commentaires</Text>
        </Pressable>
        <Text fontWeight="light" fontSize="xs">{new Date(post.created_at).toLocaleString('fr-FR', options)}</Text>
      </VStack>
      {
        showLikers && (
          <Modal
